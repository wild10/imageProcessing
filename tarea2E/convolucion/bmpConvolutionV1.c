...
void crear_imagen(BMP *imagen, char ruta[],int escala){
	...
	BMP imgNew;
	unsigned char** imgScGris;
	unsigned char** imgScGris1;
	unsigned char** imgScGris2;
	unsigned char** imgScGris11;
	unsigned char** imgScGris22;
	unsigned char** imgBorde;
	...
	//escala de grises
	imgScGris=malloc (imgNew.alto* sizeof(char *)); 
	for( i=0; i<imgNew.alto; i++)
		imgScGris[i]=malloc (imgNew.ancho* sizeof(char));

	imgScGris1=malloc (imgNew.alto* sizeof(char *)); 
	for( i=0; i<imgNew.alto; i++)
		imgScGris1[i]=malloc (imgNew.ancho* sizeof(char));

	imgScGris2=malloc (imgNew.alto* sizeof(char *)); 
	for( i=0; i<imgNew.alto; i++)
		imgScGris2[i]=malloc (imgNew.ancho* sizeof(char));

	imgScGris11=malloc (imgNew.alto* sizeof(char *)); 
	for( i=0; i<imgNew.alto; i++)
		imgScGris11[i]=malloc (imgNew.ancho* sizeof(char));

	imgScGris22=malloc (imgNew.alto* sizeof(char *)); 
	for( i=0; i<imgNew.alto; i++)
		imgScGris22[i]=malloc (imgNew.ancho* sizeof(char));

	imgBorde=malloc (imgNew.alto* sizeof(char *)); 
	for( i=0; i<imgNew.alto; i++)
		imgBorde[i]=malloc (imgNew.ancho* sizeof(char));
	
	//matriz máscara de borde / filtro sobel 3x3 Dx 
	/*
	-1	0 	1	[1 0 -1]
	-2 	0 	2	[1
	-1 	0 	1	 2
				 1]
	*/
	//matriz máscara de borde / filtro sobel 3x3 Dy
	/*
	-1	-2	-1	[1 2 1]
	0 	0 	0	[-1
	1	2 	1	  0
	...
	*/
	...
	float sobelxDx[3]={1,0,-1};
	float sobelyDx[3]={1,2,1};
	float sobelxDy[3]={1,2,1};
	float sobelyDy[3]={-1,0,1};

	//como queremos detectar bordes pasamos la imagen a escala de grises
	for(i=0;i<imgNew.alto;i++){
		for(j=0;j<imgNew.ancho;j++){
			imgScGris[i][j]=((imagen->pixelB[i][j])+(imagen->pixelG[i][j])+(imagen->pixelR[i][j]))/3;
		}
	}
	...
	for(i=0;i<imgNew.alto;i++){
		for(j=1;j<imgNew.ancho-1;j++){
			for(int k=0;k<3;k++){
				imgScGris1[i][j]=((sobelxDx[0]*imgScGris[i][j-1]+sobelxDx[1]*imgScGris[i][j]+sobelxDx[2]*imgScGris[i][j+1])+255)/2;
				imgScGris2[i][j]=(sobelxDy[0]*imgScGris[i][j-1]+sobelxDy[1]*imgScGris[i][j]+sobelxDy[2]*imgScGris[i][j+1])/4;
			}
		}
	}
	...
	for(i=1;i<imgNew.alto-1;i++){
		for(j=0;j<imgNew.ancho;j++){
			for(int k=0;k<3;k++){
				imgScGris11[i][j]=(sobelyDx[0]*imgScGris1[i-1][j]+sobelyDx[1]*imgScGris1[i][j]+sobelyDx[2]*imgScGris1[i+1][j])/4;
				imgScGris22[i][j]=((sobelyDy[0]*imgScGris2[i-1][j]+sobelyDy[1]*imgScGris2[i][j]+sobelyDy[2]*imgScGris2[i+1][j])+255)/2;
			}
		}
	}
	...
	for(i=1;i<imgNew.alto-1;i++){
		for(j=0;j<imgNew.ancho;j++){
			imgBorde[i][j]=sqrt((float)(imgScGris11[i][j]*imgScGris11[i][j]+imgScGris22[i][j]*imgScGris22[i][j]))/360;
		}
	}
	*/
	...
	for(i=0;i<imgNew.alto;i++){
		for(j=0;j<imgNew.ancho;j++){
			imgNew.pixelB[i][j]=imgScGris11[i][j];
			imgNew.pixelG[i][j]=imgScGris11[i][j];
			imgNew.pixelR[i][j]=imgScGris11[i][j];
			/*for(int k=0;k<3;k++){
