...
#include <cmath>
using namespace std;
#define 	IMAGEN_TRATADA1	"imgerosion.bmp"		//Ruta y nombre del archivo de la imagen de salida BMP
#define 	IMAGEN_TRATADA2	"tratadaRestaurada.bmp"		//Ruta y nombre del archivo de la imagen de salida BMP

//g++ -std=c++11 erosion.cpp -o erosion
//./erosion letraA.bmp
typedef struct BMP{
...
void crear_imagenErosion(BMP *imagen, char ruta[]);	//Función para crear una imagen BMP
...
int main (int argc, char* argv[]){	
	...
	crear_imagenErosion(&img,imagen_tratada1);
...
//Parametro que devuelve: Ninguno
//****************************************************************************************************************************************************
void crear_imagenErosion(BMP *imagen, char ruta[]){
	FILE *archivo;	//Puntero FILE para el archivo de imágen a abrir
	...
	for(int k=0;k<it;k++){
		...
		for (i=1;i<imgNew.alto-1;i++){
			for (j=1;j<imgNew.ancho-1;j++){
				if(imgNew.pixelB[i][j]==0 && imgNew.pixelB[i][j-1]==0 && imgNew.pixelB[i][j+1]==0 && imgNew.pixelB[i-1][j]==0 && imgNew.pixelB[i+1][j]==0){
					stencil[i][j]=1;
				}
			}
		}
